...
def save_csv(dir_out, list_of_files, result):
    try:
        np.savetxt(f"{dir_out}Results_from_{len(list_of_files)}-files.csv",
                   result.T, delimiter=",", header='Time, Avrg_int, SD, SE, Sum_int')
    except:
...
#        mean_of_stacks = np.rint(np.mean(new_stack, axis = 0)) # rounding float to float
        ...
        save_tif(dir_out, list_of_files, slice_t, sum_of_stacks, 'Sum')

        result_csv = np.array([t_points, list_of_mean, list_of_sd, list_of_sem, list_of_sum])
